...
Codex pet package は `assets/pets/<character-id>/` に保存します。各 package には runtime file と QA output をまとめます。
...
package index は `metadata/pets.csv` です。
...
validation script は、animated WebP preview が frame manifest に記録された sprite-sheet states をすべて含むことを確認します。
